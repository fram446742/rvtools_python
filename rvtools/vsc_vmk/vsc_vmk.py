                ...
                for vnic in host.config.network.vnic:
                    ...
                    if vnic.spec and vnic.spec.ip:
                        ...
                        if hasattr(vnic.spec.ip, "dhcp"):
                            dhcp = (
                                str(vnic.spec.ip.dhcp) if vnic.spec.ip.dhcp else "False"
                            )
                    ...
                    if vnic.spec and hasattr(vnic.spec, "ipV6"):
                        ...
                        if ipv6_config:
                            if (
                                hasattr(ipv6_config, "ipAddress")
                                and ipv6_config.ipAddress
                            ):
                                if (
                                    isinstance(ipv6_config.ipAddress, list)
                                    and len(ipv6_config.ipAddress) > 0
                                ):
                                    ip_6_address = (
                                        ipv6_config.ipAddress[0].ipAddress or ""
                                    )
                    ...
                    if host.config.network and host.config.network.ipRouteConfig:
                        ...
                        if (
                            hasattr(route_config, "defaultGateway")
                            and route_config.defaultGateway
                        ):
                            gateway = route_config.defaultGateway or ""
                        if (
                            hasattr(route_config, "ipV6DefaultGateway")
                            and route_config.ipV6DefaultGateway
                        ):
