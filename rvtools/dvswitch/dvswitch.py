...
class DVSwitchCollector(BaseCollector):
    ...
    def _collect_dvswitch(self, dvswitch):
        ...
        dvs_data["created"] = (
            str(dvswitch.config.creationTime) if dvswitch.config else ""
        )

        host_members = (
            len(dvswitch.config.host) if dvswitch.config and dvswitch.config.host else 0
        )
        dvs_data["host_members"] = str(host_members)

        dvs_data["max_ports"] = str(dvswitch.config.maxPorts) if dvswitch.config else ""
        dvs_data["num_ports"] = (
            str(len(dvswitch.portgroup)) if dvswitch.portgroup else "0"
        )
        dvs_data["num_vms"] = ""
