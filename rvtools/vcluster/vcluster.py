...
class VClusterCollector(BaseCollector):
    ...
    def _collect_cluster(self, cluster):
        ...
        cluster_data['config_status'] = str(cluster.configStatus) if cluster.configStatus else ""
        cluster_data['overall_status'] = str(cluster.overallStatus) if cluster.overallStatus else ""
        ...
        cluster_data['total_cpu'] = str(summary.totalCpu) if summary and summary.totalCpu else ""
        cluster_data['num_cpu_cores'] = str(summary.numCpuCores) if summary and summary.numCpuCores else ""
        cluster_data['num_cpu_threads'] = str(summary.numCpuThreads) if summary and summary.numCpuThreads else ""
        cluster_data['effective_cpu'] = str(summary.effectiveCpu) if summary and summary.effectiveCpu else ""
        cluster_data['total_memory'] = str(summary.totalMemory) if summary and summary.totalMemory else ""
        cluster_data['effective_memory'] = str(summary.effectiveMemory) if summary and summary.effectiveMemory else ""

        cluster_data['num_vmotions'] = ""
        
        config = cluster.configurationEx if hasattr(cluster, 'configurationEx') else None
        ...
        cluster_data['admission_control_enabled'] = str(das_config.admissionControlEnabled) if das_config else ""
        ...
        cluster_data['drs_default_vm_behavior'] = str(drs_config.defaultVmBehavior) if drs_config else ""
        cluster_data['drs_vmotion_rate'] = str(drs_config.vmotionRate) if drs_config else ""
