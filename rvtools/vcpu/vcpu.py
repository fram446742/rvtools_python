...
class VCPUCollector(BaseCollector):
    ...
    def _collect_vm_cpu(self, vm):
        ...
        cpu_data['powerstate'] = str(vm.runtime.powerState) if vm.runtime.powerState else ""
        cpu_data['template'] = str(vm.config.template) if vm.config.template else ""
        cpu_data['srm_placeholder'] = ""
        
        cpu_data['cpus'] = str(vm.config.hardware.numCPU) if vm.config.hardware.numCPU else ""
        cpu_data['sockets'] = str(vm.config.hardware.numCoresPerSocket) if hasattr(vm.config.hardware, 'numCoresPerSocket') else ""
        ...
        cpu_data['level'] = ""
        cpu_data['shares'] = str(vm.config.cpuAllocation.shares.shares) if vm.config.cpuAllocation.shares else ""
        cpu_data['reservation'] = str(vm.config.cpuAllocation.reservation) if vm.config.cpuAllocation.reservation else ""
        cpu_data['entitlement'] = ""
        cpu_data['drs_entitlement'] = ""
        cpu_data['limit'] = str(vm.config.cpuAllocation.limit) if vm.config.cpuAllocation.limit else ""
