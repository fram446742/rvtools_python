...
class VMemoryCollector(BaseCollector):
    ...
    def _collect_vm_memory(self, vm):
        ...
        memory_data['powerstate'] = str(vm.runtime.powerState) if vm.runtime.powerState else ""
        memory_data['template'] = str(vm.config.template) if vm.config.template else ""
        memory_data['srm_placeholder'] = ""
        
        memory_data['size_mib'] = str(vm.config.hardware.memoryMB) if vm.config.hardware.memoryMB else ""
        ...
        memory_data['level'] = ""
        memory_data['shares'] = str(vm.config.memoryAllocation.shares.shares) if vm.config.memoryAllocation.shares else ""
        memory_data['reservation'] = str(vm.config.memoryAllocation.reservation) if vm.config.memoryAllocation.reservation else ""
        memory_data['limit'] = str(vm.config.memoryAllocation.limit) if vm.config.memoryAllocation.limit else ""
