            ...
            if dvswitch.portgroup:
                for portgroup in dvswitch.portgroup:
                    ...
                    if portgroup.config and portgroup.config.defaultPortConfig:
                        ...
                        if security:
                            allow_promiscuous = str(getattr(security, 'allowPromiscuous', ''))
                            mac_changes = str(getattr(security, 'macChanges', ''))
                            forged_transmits = str(getattr(security, 'forgedTransmits', ''))
                        ...
                        if out_shaping:
                            out_traffic_shaping = str(getattr(out_shaping, 'enabled', ''))
                        ...
                        if portgroup.config and hasattr(portgroup.config, 'configVersion'):
                            config_version = portgroup.config.configVersion
                        
                        # Port count
                        num_ports = str(len(portgroup.portKeys)) if hasattr(portgroup, 'portKeys') and portgroup.portKeys else ""
                    ...
                    port_data = {
                        ...
                        "full_duplex": "",
                        "blocked": "",
                        "allow_promiscuous": allow_promiscuous if 'allow_promiscuous' in locals() else "",
                        "mac_changes": mac_changes if 'mac_changes' in locals() else "",
                        "active_uplink": active_uplink if 'active_uplink' in locals() else "",
                        "standby_uplink": standby_uplink if 'standby_uplink' in locals() else "",
                        "policy": policy if 'policy' in locals() else "",
                        "forged_transmits": forged_transmits if 'forged_transmits' in locals() else "",
                        "in_traffic_shaping": in_traffic_shaping if 'in_traffic_shaping' in locals() else "",
                        ...
                        "out_traffic_shaping": out_traffic_shaping if 'out_traffic_shaping' in locals() else "",
                        ...
                        "reverse_policy": reverse_policy if 'reverse_policy' in locals() else "",
                        "notify_switch": notify_switch if 'notify_switch' in locals() else "",
                        "rolling_order": rolling_order if 'rolling_order' in locals() else "",
                        "check_beacon": check_beacon if 'check_beacon' in locals() else "",
                        "live_port_moving": live_port_moving if 'live_port_moving' in locals() else "",
                        "check_duplex": check_duplex if 'check_duplex' in locals() else "",
                        "check_error_percent": "",
                        "check_speed": check_speed if 'check_speed' in locals() else "",
                        "percentage": "",
                        "block_override": block_override if 'block_override' in locals() else "",
                        "config_reset": "",
                        "shaping_override": shaping_override if 'shaping_override' in locals() else "",
                        "vendor_config_override": vendor_override if 'vendor_override' in locals() else "",
                        "sec_policy_override": sec_policy_override if 'sec_policy_override' in locals() else "",
                        "teaming_override": teaming_override if 'teaming_override' in locals() else "",
                        "vlan_override": vlan_override if 'vlan_override' in locals() else "",
