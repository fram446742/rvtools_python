...
class VRPCollector(BaseCollector):
    ...
    def _collect_resource_pool(self, rp):
        ...
        rp_data['cpu_limit'] = str(cpu_alloc.limit) if cpu_alloc and cpu_alloc.limit else ""
        rp_data['cpu_overhead_limit'] = ""
        rp_data['cpu_reservation'] = str(cpu_alloc.reservation) if cpu_alloc and cpu_alloc.reservation else ""
        rp_data['cpu_level'] = ""
        rp_data['cpu_shares'] = str(cpu_alloc.shares.shares) if cpu_alloc and cpu_alloc.shares else ""
        rp_data['cpu_expandable_reservation'] = str(cpu_alloc.expandableReservation) if cpu_alloc else ""
        ...
        rp_data['mem_reservation'] = str(mem_alloc.reservation) if mem_alloc and mem_alloc.reservation else ""
        rp_data['mem_level'] = ""
        rp_data['mem_shares'] = str(mem_alloc.shares.shares) if mem_alloc and mem_alloc.shares else ""
        rp_data['mem_expandable_reservation'] = str(mem_alloc.expandableReservation) if mem_alloc else ""
